// 设置根目录
var baseURL = './';

window.onload = function () {
    // 获取高度
    var winW = window.innerWidth;
    var winH = window.innerHeight;

    //添加背景
    var addBackgroundImg;

    // 创建游戏实例
    var game = new Phaser.Game(winW, winH, Phaser.AUTO, '#app');

    // 定义场景
    var states = {
        // 加载场景
        preload: function () {
            this.preload = function () {
                // 设置背景颜色
                game.stage.backgroundColor = '#000000';

                // 加载资源
                game.load.crossOrigin = 'anonymous'; // 设置跨域
                game.load.image('bg', baseURL + 'images/bg.png');
                game.load.image('dude', baseURL + 'images/dude.png');
                game.load.image('green', baseURL + 'images/green.png');
                game.load.image('red', baseURL + 'images/red.png');
                game.load.image('yellow', baseURL + 'images/yellow.png');
                game.load.image('bomb', baseURL + 'images/bomb.png');
                game.load.image('five', baseURL + 'images/five.png');
                game.load.image('three', baseURL + 'images/three.png');
                game.load.image('one', baseURL + 'images/one.png');
                game.load.audio('bgMusic', baseURL + 'audio/bgMusic.mp3');
                game.load.audio('scoreMusic', baseURL + 'audio/addscore.mp3');
                game.load.audio('bombMusic', baseURL + 'audio/boom.mp3');

                // 添加进度文字
                var progressText = game.add.text(game.world.centerX, game.world.centerY, '0%', {
                    fontSize: '60px',
                    fill: '#ffffff'
                });

                // 设置锚点，用于居中
                progressText.anchor.setTo(0.5, 0.5);

                // 监听加载完一个文件的事件
                game.load.onFileComplete.add(function (progress) {
                    // console.log(progress)
                    progressText.text = progress + '%';
                });

                // 监听加载完毕事件
                game.load.onLoadComplete.add(onLoad);

                // 最小展示时间，示例为3秒
                var deadLine = false;
                setTimeout(function () {
                    deadLine = true;
                }, 1000);

                // 加载完毕回调方法
                function onLoad() {
                    if (deadLine) {
                        // 已到达最小展示时间，可以进入下一个场景
                        game.state.start('created');
                    } else {
                        // 还没有到最小展示时间，1秒后重试
                        setTimeout(onLoad, 1000);
                    }
                }
            };



            // this.create = function () {
            //     alert('加载完成！');
            // };
        },
        // 开始场景
        created: function () {
            this.create = function () {
                addBackgroundImg = function () {
                    // 添加背景
                    var bg = game.add.image(0, 0, 'bg');
                    // 设置宽高
                    bg.width = game.world.width;
                    bg.height = game.world.height;
                };
                addBackgroundImg();

                // 添加标题
                var title = game.add.text(game.world.centerX, game.world.height * 0.25, '小恐龙接苹果', {
                    fontSize: '40px',
                    fontWeight: 'bold',
                    fill: '#f2bb15'
                });
                // 设置标题位置
                title.anchor.setTo(0.5, 0.5);

                // 添加提示
                var remind = game.add.text(game.world.centerX, game.world.centerY, '点击任意位置开始', {
                    fontSize: '20px',
                    fill: '#f2bb15'
                });
                remind.anchor.setTo(0.5, 0.5);

                // 添加主角
                var man = game.add.sprite(game.world.centerX, game.world.height * 0.75, 'dude');

                // 从缓存中获取游戏对象图片
                var manImage = game.cache.getImage('dude');

                // 设置游戏对象图片宽高
                man.width = game.world.width * 0.2;
                man.height = man.width / manImage.width * manImage.height;

                // 设置游戏对象位置
                man.anchor.setTo(0.5, 0.5);

                // 添加点击事件
                game.input.onTap.add(function () {
                    game.state.start('play');
                });
            }
        },
        // 游戏场景
        play: function () {
            var man; // 主角
            var apples; // 苹果
            var score = 0; // 得分
            var title; // 分数
            var scoreMusic;
            var bombMusic;
            var bgMusic;
            this.create = function () {
                score = 0;

                addBackgroundImg();

                // 开启物理引擎
                game.physics.startSystem(Phaser.Physics.Arcade);
                game.physics.arcade.gravity.y = 500;

                if (!bgMusic) {
                    // 添加背景音乐
                    var bgMusic = game.add.audio('bgMusic');
                    // 循环播放
                    bgMusic.loopFull();
                }


                // 缓存其他音乐
                scoreMusic = game.add.audio('scoreMusic');
                bombMusic = game.add.audio('bombMusic');


                // 添加主角
                man = game.add.sprite(game.world.centerX, game.world.height * 0.75, 'dude');
                var manImage = game.cache.getImage('dude');
                man.width = game.world.width * 0.2;
                man.height = man.width / manImage.width * manImage.height;
                man.anchor.setTo(0.5, 0.5);
                // 加入物理运动
                game.physics.enable(man);
                // 清除重力影响
                man.body.allowGravity = false;

                // 添加分数
                title = game.add.text(game.world.centerX, game.world.height * 0.25, '0', {
                    fontSize: '40px',
                    fontWeight: 'bold',
                    fill: '#f2bb15'
                });
                title.anchor.setTo(0.5, 0.5);

                // 主角移动
                // 是否正在触摸
                var touching = false;
                // 监听按下事件
                game.input.onDown.add(function (pointer) {
                    if (Math.abs(pointer.x - man.x) < man.width / 2) touching = true;
                });
                // 监听离开事件
                game.input.onUp.add(function () {
                    touching = false;
                });
                // 监听滑动事件
                game.input.addMoveCallback(function (pointer, x, y, isTap) {
                    if (!isTap && touching) man.x = x;
                });

                // 添加苹果组
                apples = game.add.group();

                // 苹果类型
                var appleTypes = ['green', 'red', 'yellow', 'bomb'];

                // 定时产生苹果
                var appleTimer = game.time.create(true);
                appleTimer.loop(1000, function () {
                    // 随机产生随机苹果
                    var x = Math.random() * game.world.width;
                    var y = Math.random() * game.world.height;
                    var type = appleTypes[Math.floor(Math.random() * appleTypes.length)];
                    var apple = apples.create(x, 0, type);

                    // 设置苹果大小
                    var appleImg = game.cache.getImage(type);
                    apple.width = game.world.width / 8;
                    apple.height = apple.width / appleImg.width * appleImg.height;

                    // 设置苹果加入物理运动
                    game.physics.enable(apple);

                    // 设置苹果与游戏边缘碰撞，
                    apple.body.collideWorldBounds = true;
                    apple.body.onWorldBounds = new Phaser.Signal();
                    apple.body.onWorldBounds.add(function (apple, up, down, left, right) {
                        if (down) {
                            apple.kill();
                            if (apple.key !== 'bomb') {
                                game.state.start('over', true, false, score);
                            }
                        }
                    });
                });
                // 定时器开始
                appleTimer.start();

            };
            this.update = function () {
                // 监听接触事件
                game.physics.arcade.overlap(man, apples, pickApple, null, this);
            };

            // 接触事件
            function pickApple(man, apple) {
                if (apple.key === 'bomb') {
                    // 播放音效
                    bombMusic.play();
                    
                    game.state.start('over', true, false, score);
                } else {
                    // 单个苹果几分项
                    var point = 1;
                    // 接到苹果产生的效果
                    var img = 'one';
                    if (apple.key === 'red') {
                        point = 3;
                        img = 'three';
                    } else if (apple.key === 'yellow') {
                        point = 5;
                        img = 'five';
                    }
                }

                // 添加得分图片
                var goal = game.add.image(apple.x, apple.y, img);
                var goalImg = game.cache.getImage(img);
                goal.width = apple.width;
                goal.height = goal.width / (goalImg.width / goalImg.height);
                goal.alpha = 0;

                // 添加过渡效果
                var showTween = game.add.tween(goal).to({
                    alpha: 1,
                    y: goal.y - 20
                }, 100, Phaser.Easing.Linear.None, true, 0, 0, false);
                showTween.onComplete.add(function () {
                    var hideTween = game.add.tween(goal).to({
                        alpha: 0,
                        y: goal.y - 20
                    }, 100, Phaser.Easing.Linear.None, true, 200, 0, false);
                    hideTween.onComplete.add(function () {
                        goal.kill();
                    });
                });

                // 更新分数
                score += point;
                title.text = score;
                // 清除苹果
                apple.destroy();
                // 播放音效
                scoreMusic.play();
            }
        },
        // 结束场景
        over: function () {
            // 得分
            var score = 0;

            this.init = function () {
                score = arguments[0];
            };

            this.create = function () {

                addBackgroundImg();

                // 添加文本
                var title = game.add.text(game.world.centerX, game.world.height * 0.25, '游戏结束', {
                    fontSize: '40px',
                    fontWeight: 'bold',
                    fill: '#f2bb15'
                });
                title.anchor.setTo(0.5, 0.5);


                // 计算总分
                var scoreStr = '你的得分是：' + score + '分';
                var scoreText = game.add.text(game.world.centerX, game.world.height * 0.4, scoreStr, {
                    fontSize: '30px',
                    fontWeight: 'bold',
                    fill: '#f2bb15'
                });
                scoreText.anchor.setTo(0.5, 0.5);
                var remind = game.add.text(game.world.centerX, game.world.height * 0.6, '点击任意位置再玩一次', {
                    fontSize: '20px',
                    fontWeight: 'bold',
                    fill: '#f2bb15'
                });
                remind.anchor.setTo(0.5, 0.5);

                // 添加点击事件
                game.input.onTap.add(function () {
                    game.state.start('play');
                });
            };
        }
    };

    // 添加场景到游戏示例中
    Object.keys(states).map(function (key) {
        // console.log(key)
        game.state.add(key, states[key]);
    });

    game.state.start('preload');

};